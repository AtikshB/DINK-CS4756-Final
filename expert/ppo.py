...
class PPOAgent:
    def __init__(
        ...
        # Actor-Network
        self.actor_net = actor_m.to(self.device)
        self.actor_optimizer = optim.Adam(self.actor_net.parameters(), lr=self.alpha)

        # Critic-Network
        self.critic_net = critic_m.to(self.device)
        ...
        for _ in range(self.ppo_epoch):
            for state, action, old_log_probs, return_, advantage in self.ppo_iter(
                ...
                if loss < self.best_loss:
                    self.save_model("../models/expert_actor.pth")
                    self.best_loss = loss
    ...
    def load_model(self, path, device):
        self.actor_net = ActorCnn(self.input_shape, self.action_size).to(device)
